...
fn main() {
    ...
    println!(
        "  Single engine flow:   {:.1} kg/s",
        mass_flow_rate(thrust_sl, isp_sl)
    );
    ...
    let dv_land = tsiolkovsky_delta_v(
        isp_sl,
        m_before_landing / (m_before_landing - fuel_used_land),
    );
    ...
    println!("\n═══ MISSION SUMMARY ═══");
    println!(
        "Total fuel consumed:    {:.0} kg ({:.1}% of landing fuel)",
        total_fuel_used,
        (total_fuel_used / m_prop_landing) * 100.0
    );
    println!("Total ΔV used:          {:.0} m/s", total_dv_used);
    println!(
        "Remaining fuel:         {:.0} kg",
        m_prop_landing - total_fuel_used
    );
    ...
    if total_fuel_used <= m_prop_landing {
        ...
        println!(
            "⚠️  FUEL SHORTAGE:      {:.0} kg over budget",
            total_fuel_used - m_prop_landing
        );
    }
    ...
    println!(
        "Hover capability:       {}",
        if final_twr > 1.0 { "YES" } else { "NO" }
    );
    println!(
        "Max deceleration:       {:.1} m/s² ({:.1} g)",
        max_deceleration,
        max_deceleration / G0
    );
    ...
    println!(
        "  Estimated fuel used: ~{:.0} kg ({:.1}%)",
        total_fuel_used,
        (total_fuel_used / 133000.0) * 100.0
    ); // 133t first stage prop mass
