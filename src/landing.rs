...
impl PDGSolver {
    ...
    pub fn solve(&self) -> Result<PDGSolution, String> {
        ...
        let mut thrust = vec![Vector3::new(0.0, 0.0, self.problem.thrust_min); self.problem.num_nodes];
        ...
        while iteration < self.max_iterations && !converged {
            ...
            match self.solve_convex_subproblem(tf, &position, &velocity, &thrust, &mass) {
                ...
                Err(e) => {
                    return Err(format!("Convex subproblem failed at iteration {}: {}", iteration, e));
                }
            }
        }
    ...
    fn velocity_guess_from_position(&self, position: &[Vector3<f64>], dt: f64) -> Vec<Vector3<f64>> {
    ...
    fn mass_trajectory_guess(&self, tf: f64) -> Vec<f64> {
        ...
        (0..self.problem.num_nodes)
            .map(|i| {
                ...
                (self.problem.m0 - mass_flow_rate * t).max(self.problem.m0 * 0.1) // Minimum 10% of initial mass
            })
            .collect()
    }
    ...
    fn solve_convex_subproblem(
        ...
        mass_guess: &[f64],
    ) -> Result<(f64, Vec<Vector3<f64>>, Vec<Vector3<f64>>, Vec<Vector3<f64>>, Vec<f64>), String> {
...
pub mod analysis {
    ...
    pub fn trajectory_statistics(solution: &PDGSolution) -> TrajectoryStats {
        let max_acceleration = solution.thrust.iter()
            .zip(&solution.mass)
            .map(|(t, m)| t.magnitude() / m)
            .fold(0.0, f64::max);
        
        let max_thrust = solution.thrust.iter()
            .map(|t| t.magnitude())
            .fold(0.0, f64::max);
        
        let min_thrust = solution.thrust.iter()
            .map(|t| t.magnitude())
            .filter(|&t| t > 1e-6)
            .fold(f64::INFINITY, f64::min);
        
        let max_velocity = solution.velocity.iter()
            .map(|v| v.magnitude())
            .fold(0.0, f64::max);
        
        let max_altitude = solution.position.iter()
            .map(|p| p.z)
            .fold(f64::NEG_INFINITY, f64::max);
        
        let downrange_distance = {
            let start_xy = Vector3::new(solution.position[0].x, solution.position[0].y, 0.0);
            let end_xy = Vector3::new(solution.position.last().unwrap().x, solution.position.last().unwrap().y, 0.0);
    ...
    pub fn validate_constraints(problem: &PDGProblem, solution: &PDGSolution) -> ConstraintViolations {
        ...
        for (i, ((thrust, mass), pos)) in solution.thrust.iter()
            ...
            if thrust_mag > problem.thrust_max + 1e-6 {
                violations.thrust_max_violations.push((i, thrust_mag - problem.thrust_max));
            }
            if thrust_mag > 1e-6 && thrust_mag < problem.thrust_min - 1e-6 {
                violations.thrust_min_violations.push((i, problem.thrust_min - thrust_mag));
            }
            ...
            if thrust_mag > 1e-6 {
                ...
                if tilt_angle > problem.max_tilt_angle + 1e-6 {
                    violations.tilt_violations.push((i, tilt_angle - problem.max_tilt_angle));
                }
            }
            ...
            if altitude < required_altitude - 1e-6 {
                violations.glideslope_violations.push((i, required_altitude - altitude));
            }
        }
...
mod tests {
    ...
    fn test_constraint_validation() {
        ...
        let mut solution = PDGSolution {
            ...
            velocity: vec![v0, Vector3::new(0.0, 0.0, -25.0), Vector3::new(0.0, 0.0, -5.0)],
            thrust: vec![
    ...
    fn test_trajectory_statistics() {
        ...
        let solution = PDGSolution {
            ...
            thrust: vec![Vector3::new(0.0, 0.0, 400000.0), Vector3::new(0.0, 0.0, 500000.0)],
