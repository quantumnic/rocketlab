...
impl EntrySimulator {
    ...
    pub fn propagate_ballistic_entry(&self, analysis: &EntryAnalysis) -> Result<EntryTrajectory, String> {
    ...
    fn calculate_heating(&self, state: &EntryState) -> EntryHeating {
        ...
        let emissivity = self.vehicle.tps.as_ref().map(|tps| tps.emissivity).unwrap_or(0.85);
