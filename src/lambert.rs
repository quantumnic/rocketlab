...
pub fn solve_lambert(
    ...
    if sin_dnu.abs() < 1e-10 {
        ...
        return Err(
            "180° transfer: orbit plane undetermined, add out-of-plane component".to_string(),
        );
    }
    ...
    let _f_and_fp = |z: f64| -> (f64, f64) {
        ...
        let f_prime = if z.abs() > 1e-6 {
            let dy_dz = if z.abs() > 1e-6 {
                ...
                let y_plus = r1_mag
                    + r2_mag
                    + a_param * ((z + eps) * stumpff_s(z + eps) - 1.0) / stumpff_c(z + eps).sqrt();
                let y_minus = r1_mag
                    + r2_mag
                    + a_param * ((z - eps) * stumpff_s(z - eps) - 1.0) / stumpff_c(z - eps).sqrt();
            ...
            // z ≈ 0: use series
            (2.0 / 40.0) * y_val.powf(1.5)
                + a_param / 8.0 * (y_val.sqrt() + a_param * (1.0 / (2.0 * y_val.sqrt())))
        };
...
pub fn solve_lambert_multirev(
    ...
    for rev in 0..=revolutions.abs() {
        for &prograde in &[true, false] {
            let period_est =
                2.0 * PI * ((r1.magnitude() + r2.magnitude()) / 4.0).powf(1.5) / mu.sqrt();
...
mod tests {
    ...
    fn test_lambert_circular_orbit() {
        ...
        assert!(
            (solution.a - r).abs() / r < 0.02,
            "Semi-major axis mismatch: got {:.1}, expected ~{:.1}",
            solution.a,
            r
        );
    ...
    fn test_lambert_hohmann_transfer() {
        ...
        assert!(
            (solution.a - a_transfer).abs() < 100.0,
            "Hohmann a: got {:.1}, expected {:.1}",
            solution.a,
            a_transfer
        );
        ...
        assert!(
            (solution.e - expected_e).abs() < 0.02,
            "Hohmann e: got {:.4}, expected {:.4}",
            solution.e,
            expected_e
        );
    }
    ...
    fn test_lambert_vallado_example() {
        ...
        assert!(
            solution.a > 149597870.7,
            "Transfer orbit too small: {}",
            solution.a
        );
        assert!(
            solution.a < 300000000.0,
            "Transfer orbit too large: {}",
            solution.a
        );
        assert!(
            solution.e > 0.1,
            "Transfer should be elliptical: e={}",
            solution.e
        );
    }
    ...
    fn test_delta_v_calculation() {
        ...
        let (dv1, dv2, dv_total) =
            lambert_delta_v(r1, v1_initial, r2, v2_target, dt, MU_EARTH, true).unwrap();
        ...
        assert!(
            dv_total < 1.0,
            "ΔV too large for same-orbit transfer: {}",
            dv_total
        );
    }
}
