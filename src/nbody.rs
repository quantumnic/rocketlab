...
use crate::constants::{AU_KM, C_LIGHT, J2_EARTH, MU_EARTH, R_EARTH, TWO_PI};
...
impl NBODYPropagator {
    ...
    pub fn propagate(
        ...
        while elapsed_time < total_time && elapsed_time < self.integration.max_time {
            ...
            let (new_pos, new_vel, actual_step) = match self.integration.integrator {
                ...
                IntegratorType::DormandPrince78 => self.dp78_step(&current_state, step_size)?,
    ...
    fn compute_atmospheric_drag(
        ...
        let drag_coeff =
            0.5 * density * self.satellite.cd * self.satellite.area_drag / self.satellite.mass;
    ...
    fn compute_solar_radiation_pressure(
        ...
        let srp_accel = -solar_pressure * self.satellite.cr * self.satellite.area_srp * sun_unit
            / self.satellite.mass;
    ...
    fn compute_relativistic_acceleration(
        &self,
        r: &Vector3<f64>,
        v: &Vector3<f64>,
    ) -> Vector3<f64> {
