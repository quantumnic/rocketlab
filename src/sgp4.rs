...
impl SGP4 {
    ...
    pub fn new(tle: TLE) -> Result<Self, String> {
        ...
        let init_data = sgp4_init(
            i0, omega0, w0, m0, n0, e0, bstar, deep_space, &tle
        )?;
...
fn sgp4_init(
    ...
    let init_data = SGP4InitData {
        c1, c2,
        c3: 0.0, c4: 0.0, c5: 0.0, d2: 0.0, d3: 0.0, d4: 0.0,
        delmo: 0.0, eta: e0, omgcof: 0.0, sinmao, t2cof: 0.0, t3cof: 0.0,
        t4cof: 0.0, t5cof: 0.0, x1mth2: 0.0, x7thm1: 0.0, xlcof: 0.0,
        xmcof: 0.0, xnodcf: 0.0, aycof: 0.0, con41: 0.0, cc1: 0.0, cc4: 0.0,
        cc5: 0.0, cosio, sinio, xj2: XJ2, xj3: XJ3, xj4: XJ4, xke: XKE,
        xlamo: 0.0, xli: 0.0, xni: n0,
        // Deep space zeros
        irez: 0, d2201: 0.0, d2211: 0.0, d3210: 0.0, d3222: 0.0, d4410: 0.0,
        d4422: 0.0, d5220: 0.0, d5232: 0.0, d5421: 0.0, d5433: 0.0, dedt: 0.0,
        del1: 0.0, del2: 0.0, del3: 0.0, didt: 0.0, dmdt: 0.0, dnodt: 0.0,
        domdt: 0.0, e3: 0.0, ee2: 0.0, peo: 0.0, pgho: 0.0, pho: 0.0,
        pinco: 0.0, plo: 0.0, se2: 0.0, se3: 0.0, sgh2: 0.0, sgh3: 0.0,
        sgh4: 0.0, sh2: 0.0, sh3: 0.0, si2: 0.0, si3: 0.0, sl2: 0.0,
        sl3: 0.0, sl4: 0.0, gsto: 0.0, xfact: 0.0, xgh2: 0.0, xgh3: 0.0,
        xgh4: 0.0, xh2: 0.0, xh3: 0.0, xi2: 0.0, xi3: 0.0, xl2: 0.0,
        xl3: 0.0, xl4: 0.0, xlamo_dp: 0.0, zmol: 0.0, zmos: 0.0, atime: 0.0,
        xli_dp: 0.0, xni_dp: 0.0,
    };
...
fn epoch_to_datetime(year: u32, day: f64) -> Result<DateTime<Utc>, String> {
    ...
    let base_date = NaiveDate::from_ymd_opt(year as i32, 1, 1)
    ...
    let epoch_datetime = base_date.and_hms_opt(0, 0, 0)
        .ok_or("Invalid time")?
        + Duration::days(days) 
...
2 28654  99.0643 107.1986 0013716 344.4945  15.5477 14.12501077969186"#;
        ...
        assert!(altitude > 400.0 && altitude < 1500.0, "Unrealistic altitude: {}", altitude);
...
2 44713  53.0532 216.9843 0001357  94.8422 265.3051 15.08443313245891"#;
        ...
        let altitude = state.position.magnitude() - R_EARTH;
        assert!(altitude > 500.0 && altitude < 600.0, "Starlink altitude: {}", altitude);
    }
}